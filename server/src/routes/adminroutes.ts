...
router.post('/login', async (req: Request, res: Response) => {
  try {
    ...
    res.cookie('supabase-auth-token', session.access_token, {
      httpOnly: true,
      secure: process.env.NODE_ENV === 'production', // true for production (HTTPS), false for development (HTTP)
      sameSite: 'lax',
    ...
    if (files?.images && files.images.length > 0) {
      ...
      for (const imageFile of files.images) {
        try {
          ...
          const supabaseFilePath = await storage.uploadFile(fileContent, imageFile.originalname);
          ...
          // Store the Supabase Storage path instead of local path
          imageObjects.push({
