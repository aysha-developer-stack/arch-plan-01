...
export class SupabaseStorage implements IStorage {
  ...
  async searchPlans(
      ...
      if (filters.outdoorFeatures) {
          ...
          if (outdoorFeaturesList.length > 0) {
              outdoorFeaturesList.forEach(feature => {
                  // Chained .filter() calls enforce AND logic for multiple features
      ...
      // 3. Add indoor feature conditions (FIXED to use AND logic)  
      if (filters.indoorFeatures) {
