...
router.get('/check-auth', authenticateAdmin, async (req, res) => {
    try {
...
router.post('/plans', authenticateAdmin, upload.fields([
    ...
    try {
        ...
        if (planData.storeys)
            planData.storeys = parseInt(planData.storeys);
        if (planData.bedrooms)
            planData.bedrooms = parseInt(planData.bedrooms);
        if (planData.toilets)
            planData.toilets = parseInt(planData.toilets);
        if (planData.livingAreas)
            planData.livingAreas = parseInt(planData.livingAreas);
        if (planData.numberOfUnits)
            planData.numberOfUnits = parseInt(planData.numberOfUnits);
        if (planData.totalBuildingHeight)
            planData.totalBuildingHeight = parseFloat(planData.totalBuildingHeight);
        if (planData.roofPitch)
            planData.roofPitch = parseFloat(planData.roofPitch);
        // Convert additional numeric fields that come as strings from the frontend
        if (planData.plotLength)
            planData.plotLength = parseFloat(planData.plotLength);
        if (planData.plotWidth)
            planData.plotWidth = parseFloat(planData.plotWidth);
        if (planData.coveredArea)
            planData.coveredArea = parseFloat(planData.coveredArea);
        if (planData.lotSizeMin)
            planData.lotSizeMin = parseFloat(planData.lotSizeMin);
        if (planData.lotSizeMax)
            planData.lotSizeMax = parseFloat(planData.lotSizeMax);
        // Set default values
