...
const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  if (validImages.length > 0) {
    // Replace existing images with new selection to ensure first selected image appears first
    setUploadForm(prev => ({ ...prev, images: validImages }));
  }
};
...
const handleAddMoreImages = (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  if (validImages.length > 0) {
    // Append new images to existing ones, maintaining the original order
    ...
    <div className="w-64 bg-white/80 backdrop-blur-md shadow-lg border-r border-gray-200/50">
      <div className="p-6">
        ...
        <div className="mb-6">
          <button 
            onClick={() => {
              // Use production URL for Railway deployment
              const isProduction = window.location.hostname.includes('railway.app');
              const targetUrl = isProduction ? 'https://arch-plan-01-production.up.railway.app/app' : '/app';
                            ...
                            <TableCell className="text-right">
                              <div className="flex items-center justify-end space-x-2">
                                ...
                                <Dialog>
                                  ...
                                  <DialogContent className="max-w-4xl max-h-[90vh] overflow-hidden" aria-describedby="plan-dialog-description">
                                    <DialogHeader>
                                      <DialogTitle>{(plan as any).jobAddress || "No Address"}</DialogTitle>
                                      <DialogDescription id="plan-dialog-description">
                                        View detailed information about this architectural plan including images, specifications, and features
                                      </DialogDescription>
                                    </DialogHeader>
    ...
    <Dialog open={isPlanLightboxOpen} onOpenChange={setIsPlanLightboxOpen}>
      <DialogContent className="max-w-[95vw] max-h-[95vh] w-full h-full p-0 bg-black/95 border-0">
        <DialogTitle className="sr-only">
          Plan Image Viewer - Image {planCurrentImageIndex + 1} of {planLightboxImages.length}
        </DialogTitle>
        <DialogDescription className="sr-only">
          View and navigate through plan images. Use arrow keys or navigation buttons to browse through the image gallery.
        </DialogDescription>
